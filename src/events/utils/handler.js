...
async function handleMessage( message, oldMessage) {
	...
	let childcommand;
do {
childcommand = __.getCommand((i.args.shift() || `index`).toLowerCase(), command.commands);
	...
	command.argvOptions = command.argvOptions || {};


	command.argvOptions.alias = {
		...command.argvOptions.alias,
		p: [ "page" ],
		l: [ "length" ],
		fs: [ "forceslowmode" ],
...
i.argv = require("mri")(require('shell-quote').parse(i.args.join(" ")), command.argvOptions);
	var now = Date.now();
