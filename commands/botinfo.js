    ...
    try {
      imports.message.channel.send("Please wait, we are loading it.").then(msg => {
        ...
        var embedping = new imports.Discord.MessageEmbed()
          ...
          .setDescription(`Invite the bot [here!](https://nefomemes.herokuapp.com/kylebot/invite)\n\nFunfact: You wont be able to invite the bot since I made it private.`)
          .addFields(
            ...
            {name: "Bot's ping", value: `${msg.createdTimestamp - imports.message.createdTimestamp}ms`, inline: true},
        ...
        imports.message.channel.send(embedping).then(msgSent => {
          msg.delete();})
      });
    } catch (error) {
